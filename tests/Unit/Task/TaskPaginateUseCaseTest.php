...
use App\Core\Task\UseCases\TaskPaginateUseCase;
use PHPUnit\Framework\TestCase;

class TaskPaginateUseCaseTest extends TestCase
{
    public function test_execute_calls_repository_and_returns_result()
    {
        ...
        $mockRepository = $this->createMock(TaskPaginatetRepositoryInterface::class);
        $mockRepository->expects($this->once())
            ->method('listRepo')
            ->with($filters, $paginate)
            ->willReturn($expectedResult);

        $useCase = new TaskPaginateUseCase($mockRepository);
